"""
train_benchmark.py — Training script for the Dual-Tower benchmark model.

Usage (mirrors train.py, just swap the output dir):
    python train_benchmark.py --epochs 20 --out_dir outputs_benchmark

All flags are identical to train.py so the two runs are directly comparable.
The only real change is that DualTowerClassifier is used instead of
MoleculePairClassifier.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train the Dual-Tower benchmark model")
    ...
    parser.add_argument("--out_dir", default="outputs_benchmark",
                        help="Output directory (default: outputs_benchmark)")
    return parser.parse_args()


# ─────────────────────────────────────────────────────────────────────────────
# Utilities  (copied verbatim from train.py so the two scripts are self-contained)
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    print("=" * 60)
    print("  Dual-Tower Benchmark Model")
    print("  (Two independent GNN encoders — no weight sharing)")
    print("=" * 60)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(
            model, loaders["train"], optimizer, criterion, args.device, args.grad_clip
        )
            ...
            torch.save(
                {
                    ...
                    "model_type": "dual_tower",  # tag so compare_models.py can identify it
                },
                best_state_path,
            )
        else:
    ...
    test_probs, test_y_true = collect_predictions(model, loaders["test"], args.device)
    test_metrics = evaluate_from_predictions(test_probs, test_y_true, test_threshold)
    ...
    summary = {"best_val_pr_auc": best_val_pr_auc, "best_threshold": test_threshold}
